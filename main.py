...
st.set_page_config(page_title="GitHub Repositories List", page_icon=":computer:", layout="wide",
...
def fetch_github_repos(username):
    ...
    while True:
        ...
        # If the response status code is 404, the username is invalid
        if response.status_code == 404:
            st.error("Invalid username. Please try again.")
            st.stop()
        # If the response status code is 403, the API rate limit has been exceeded
